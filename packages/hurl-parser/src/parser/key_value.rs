...
pub fn value_parser() -> impl Parser<char, InterpolatedString, Error = Simple<char>> + Clone {
    ...
    let value_string_text = filter::<_, _, Simple<char>>(|c: &char| {
        c != &'#' && c != &'\n' && c != &'\\' 
    ...
    .at_least(1)
    .collect::<String>()
    .map(InterpolatedStringPart::Str).labelled("value-string-content");
    ...
    let value_template_part = template
        .map(|t| InterpolatedStringPart::Template(t)).labelled("value-template");
    ...
    let value_string = value_part
        .repeated()
        .at_least(1)
        .map(|v| InterpolatedString { parts: v }).labelled("value-string");
...
pub fn key_value_parser() -> impl Parser<char, KeyValue, Error = Simple<char>> + Clone {
    ...
    let key_value = key
        ...
        .then(value)
        .map(|(key, value)| KeyValue { key, value }).labelled("key-value");
